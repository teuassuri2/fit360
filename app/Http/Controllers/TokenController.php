...
public function storeApi(StoreTokenRequest $request) {if ($request->isMethod("post")) {$token = $this->tokenService->store($request->validated());return response()->json(new TokenJsonResource($token), 200);}
}
...
public function store(StoreTokenRequest $request) {if ($request->isMethod("post")) {$token = $this->tokenService->store($request->validated());}
